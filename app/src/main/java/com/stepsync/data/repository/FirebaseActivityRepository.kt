...
class FirebaseActivityRepository @Inject constructor(
    ...
    override fun getAllActivities(userId: Long): Flow<List<Activity>> = callbackFlow {
    ...
    override fun getActivitiesByType(userId: Long, type: String): Flow<List<Activity>> = callbackFlow {
        val currentUser = auth.currentUser
        if (currentUser == null) {
            trySend(emptyList())
            awaitClose { }
    ...
    override fun getRecentActivities(userId: Long, limit: Int): Flow<List<Activity>> = callbackFlow {
        val currentUser = auth.currentUser
        if (currentUser == null) {
            trySend(emptyList())
            awaitClose { }
