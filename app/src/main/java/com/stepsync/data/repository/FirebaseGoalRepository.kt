...
class FirebaseGoalRepository @Inject constructor(
    ...
    override fun getAllGoals(userId: Long): Flow<List<Goal>> = callbackFlow {
    ...
    override fun getActiveGoals(userId: Long): Flow<List<Goal>> = callbackFlow {
        val currentUser = auth.currentUser
        if (currentUser == null) {
            trySend(emptyList())
            awaitClose { }
    ...
    override fun getCompletedGoals(userId: Long): Flow<List<Goal>> = callbackFlow {
        val currentUser = auth.currentUser
        if (currentUser == null) {
            trySend(emptyList())
            awaitClose { }
